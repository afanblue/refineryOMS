      ...
      <Stage height={IMAGEHEIGHT} width={IMAGEWIDTH}>
        <Layer>
          ...
          {this.state.tags.map(
            function(n,x) {
              var loc  = n.location;
              var xnw  = sX( loc.c1Long, f.c1Long, xScale);
              var wid  = Math.abs(sX( loc.c2Long, f.c1Long, xScale) - xnw);
              var ynw  = sY( loc.c1Lat,  f.c1Lat,  yScale);
              var ht   = Math.abs(sY( loc.c2Lat,  f.c1Lat,  yScale) - ynw);
