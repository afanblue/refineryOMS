-- select t.name, ctv.name contents from tag t join tank tk on t.id=tk.id join content_type_vw ctv on ctv.code=tk.content_type_code where t.name like 'FOT08%' and t.tag_type_code='TK'
...
set @type='valve';
set @device='FOT08-Vi';
set @inpTag='FOT08-ViDI';
set @outTag='FOT08-ViDO';
set @desc='FuelOil Tk 08';
...
set @dir='OUTL';
set @device='FOT08-Vo';
set @inpTag='FOT08-VoDI';
set @outTag='FOT08-VoDO';
select 'tag' from dual;
insert into tag(name, description, tag_type_code, misc, active) values(@device, concat(@desc,' ',@type,' in'), @devType,null,'Y');
insert into tag(name, description, tag_type_code, misc, active) values(@inpTag, concat('On/Off ',@desc,@type,' in'),'DI',null,'Y');
insert into tag(name, description, tag_type_code, misc, active) values(@outTag, concat('On/Off ',@desc,@type,' in'),'DO',null,'Y');
...
set @dir='OUTL';
set @type='pump';
set @device='FOT08-Pmp';
set @inpTag='FOT08-PoDI';
set @outTag='FOT08-PoDO';
select 'tag' from dual;
insert into tag(name, description, tag_type_code, misc, active) values(@device, concat(@desc,' ',@type,' in'), @devType,null,'Y');
insert into tag(name, description, tag_type_code, misc, active) values(@inpTag, concat('On/Off ',@desc,@type,' in'),'DI',null,'Y');
insert into tag(name, description, tag_type_code, misc, active) values(@outTag, concat('On/Off ',@desc,@type,' in'),'DO',null,'Y');
