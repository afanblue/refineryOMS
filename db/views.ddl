...
CREATE VIEW order_carrier_vw AS select `x`.`id` AS `id`,group_concat(`x`.`name` separator '<br>') AS `carrier` from (select distinct `si`.`id` AS `id`,`c`.`name` AS `name` from (`oms`.`shipment_item` `si` left join `oms`.`tag` `c` on(`si`.`carrier_id` = `c`.`id`))) `x` group by `x`.`id`;
-- ---*************************** 26. row ***************************
select 'order_contents_vw' as action from dual;
DROP VIEW IF EXISTS order_contents_vw;
CREATE VIEW order_contents_vw AS select `x`.`id` AS `id`,group_concat(`x`.`content_cd` separator ',') AS `contents` from (select distinct `si`.`id` AS `id`,`si`.`content_cd` AS `content_cd` from `oms`.`shipment_item` `si`) `x` group by `x`.`id`;
-- ---*************************** 27. row ***************************
