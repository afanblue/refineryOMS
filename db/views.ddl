...
CREATE VIEW order_volume_vw AS select `oms`.`item`.`shipment_id` AS `shipment_id`,sum(`oms`.`item`.`exp_volume_max`) AS `exp_volume`,sum(`oms`.`item`.`act_volume`) AS `act_volume` from `oms`.`item` group by `oms`.`item`.`shipment_id`;
-- ---*************************** 21. row ***************************
