...
CREATE VIEW active_order_vw AS select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,count(0) AS `sc` from `oms`.`shipment_item` where `oms`.`shipment_item`.`active` not in ('C','D') group by `oms`.`shipment_item`.`shipment_id` union select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,if(count(0) > 0,0,0) AS `sc` from `oms`.`shipment_item` where `oms`.`shipment_item`.`active` in ('C','D') group by `oms`.`shipment_item`.`shipment_id`;
-- ---*************************** 2. row ***************************
select 'active_tank_vw' as action from dual;
DROP VIEW IF EXISTS active_tank_vw;
CREATE VIEW active_tank_vw AS select `xfr`.`source_id` AS `end_point_id` from (`oms`.`transfer` `xfr` join `oms`.`transfer_status_vw` `tsv` on(`xfr`.`status_id` = `tsv`.`value`)) where `tsv`.`code` in ('A','S') union select `xfr`.`destination_id` AS `end_point_id` from (`oms`.`transfer` `xfr` join `oms`.`transfer_status_vw` `tsv` on(`xfr`.`status_id` = `tsv`.`value`)) where `tsv`.`code` in ('A','S');
-- ---*************************** 3. row ***************************
...
CREATE VIEW alarm_color_list_vw AS select 1 AS `id`,substr(`oms`.`config`.`item_name`,1,octet_length(`oms`.`config`.`item_name`) - 5) AS `item_name`,`oms`.`config`.`item_value` AS `item_value` from `oms`.`config` where `oms`.`config`.`item_name` like '%COLOR';
-- ---*************************** 6. row ***************************
...
-- ---*************************** 12. row ***************************
select 'complete_order_vw' as action from dual;
DROP VIEW IF EXISTS complete_order_vw;
CREATE VIEW complete_order_vw AS select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,count(0) AS `sc` from `oms`.`shipment_item` where `oms`.`shipment_item`.`active` in ('C','D') group by `oms`.`shipment_item`.`shipment_id` union select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,if(count(0) > 0,0,0) AS `sc` from `oms`.`shipment_item` where `oms`.`shipment_item`.`active` not in ('C','D') group by `oms`.`shipment_item`.`shipment_id`;
-- ---*************************** 13. row ***************************
...
-- ---*************************** 19. row ***************************
select 'history_vw' as action from dual;
DROP VIEW IF EXISTS history_vw;
CREATE VIEW history_vw AS select `h`.`id` AS `id`,`t`.`id` AS `tag_id`,`t`.`name` AS `name`,round(`h`.`scan_value`,4) AS `scan_value`,from_unixtime(`h`.`scan_time`) AS `scan_time` from (`oms`.`tag` `t` join `oms`.`history` `h` on(`t`.`id` = `h`.`tag_id`));
-- ---*************************** 20. row ***************************
...
-- ---*************************** 21. row ***************************
select 'integer_list_vw' as action from dual;
DROP VIEW IF EXISTS integer_list_vw;
CREATE VIEW integer_list_vw AS select `c`.`id` - `m`.`cmin` AS `ordinal` from (`oms`.`config` `c` join (select min(`oms`.`config`.`id`) AS `cmin` from `oms`.`config`) `m`);
-- ---*************************** 22. row ***************************
...
-- ---*************************** 25. row ***************************
select 'order_carrier_vw' as action from dual;
DROP VIEW IF EXISTS order_carrier_vw;
CREATE VIEW order_carrier_vw AS select `si`.`shipment_id` AS `shipment_id`,group_concat(`c`.`name` separator '<br>') AS `carrier` from (`oms`.`shipment_item` `si` left join `oms`.`tag` `c` on(`si`.`carrier_id` = `c`.`id`)) group by `si`.`shipment_id`;
-- ---*************************** 26. row ***************************
select 'order_contents_vw' as action from dual;
DROP VIEW IF EXISTS order_contents_vw;
CREATE VIEW order_contents_vw AS select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,group_concat(`oms`.`shipment_item`.`content_cd` separator ',') AS `contents` from `oms`.`shipment_item` group by `oms`.`shipment_item`.`shipment_id`;
-- ---*************************** 27. row ***************************
...
CREATE VIEW pending_order_vw AS select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,count(0) AS `sc` from `oms`.`shipment_item` where `oms`.`shipment_item`.`active` = 'P' group by `oms`.`shipment_item`.`shipment_id` union select `oms`.`shipment_item`.`shipment_id` AS `shipment_id`,if(count(0) > 0,0,0) AS `if(count(*)>0,0,0)` from `oms`.`shipment_item` where `oms`.`shipment_item`.`active` <> 'P' group by `oms`.`shipment_item`.`shipment_id`;
-- ---*************************** 29. row ***************************
